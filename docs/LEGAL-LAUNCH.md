...
10. Conferir publicidade, simuladores e conteúdos com profissional habilitado antes de apresentar qualquer resultado como orientação individual. **Atualização do M37:** a fronteira técnica passou a existir e é verificável. O pedido enviado ao modelo proíbe recomendação de investimento, produto, instituição e alocação; a saída é filtrada por padrões determinísticos no servidor (`netlify/functions/_shared/ai-boundaries.js`) e de novo no navegador (`js/insights.js`), e toda análise sai com a natureza declarada (educativa, com valor futuro como estimativa). O que continua pendente é a **avaliação jurídica formal** desse desenho: implementar a fronteira não substitui a opinião de quem responde por ela.
