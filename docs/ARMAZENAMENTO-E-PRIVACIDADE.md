...
| Pacote do cache offline | v70 | `VERSION` em `service-worker.js` |
...
| Cache | Conteúdo |
|---|---|
| `financas-cache-v70` | HTML do app, CSS, JavaScript, PDF.js, fontes locais, manifesto e ícones |
| `financas-pages-v70` | Landing e outras navegações públicas |
| `financas-fonts-v70` | Reserva para fontes externas; vazio hoje porque `FONT_HOSTS` está vazio |

No pacote publicado, cada nome recebe ainda o SHA-256 integral da publicação
depois de `v70`. Isso faz duas versões coexistirem durante a instalação sem que
