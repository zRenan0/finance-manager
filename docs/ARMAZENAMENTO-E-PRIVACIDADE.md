...
| Pacote do cache offline | v65 | `VERSION` em `service-worker.js` |
...
| Cache | Conteúdo |
|---|---|
| `financas-cache-v65` | HTML do app, CSS, JavaScript, PDF.js, fontes locais, manifesto e ícones |
| `financas-pages-v65` | Landing e outras navegações públicas |
| `financas-fonts-v65` | Reserva para fontes externas; vazio hoje porque `FONT_HOSTS` está vazio |

No pacote publicado, cada nome recebe ainda o SHA-256 integral da publicação
depois de `v65`. Isso faz duas versões coexistirem durante a instalação sem que
